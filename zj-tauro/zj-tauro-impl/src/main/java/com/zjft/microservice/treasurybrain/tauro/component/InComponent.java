...
public class InComponent {
	...
	public String recover(TaskTransferDTO taskTransferDTO, ObjectDTO returnDTO, List<String> taskNos) {
		...
		List<String> containerNoList = taskTransferDTO.getEntityNos();

		if (StringUtil.isNullorEmpty(lineNo) || StringUtil.isNullorEmpty(StringUtil.parseString(taskType)) ||
				StringUtil.isNullorEmpty(StringUtil.parseString(nextStatus))  || containerNoList.size() == 0) {
		...
		for (String taskNo : taskList) {
			...
			for (String containerNo : list) {
				if (!containerNoList.contains(containerNo)) {
			...
			for (String containerNo : list) {
				...
				List<String> allList = inService.getBackContainerNosList(containerNo, allContainerNoList, taskNo);
